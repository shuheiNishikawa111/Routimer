    ...
    @IBAction func reloadAllTime(_ sender: Any) {
        ...
        let OK = UIAlertAction(title: "OK", style: .default, handler: { (action) -> Void in
            for i in 0 ..< tempStringList.count {
                tempIntList[i][0] = tempSaveData[i][0] * 360 + tempSaveData[i][1] * 60 + tempSaveData[i][2] * 1
            }
            self.reloadItem()
...
extension taskViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //セーブデータの合計値計算
        let totalNum = tempSaveData[indexPath.row][0] * 360 + tempSaveData[indexPath.row][1] * 60 + tempSaveData[indexPath.row][2] * 1
        
        //もしsaveDataが0秒の時、リロードボタンを打てなくする
        if ( tempSaveData[indexPath.row][0] == 0 && tempSaveData[indexPath.row][1] == 0 && tempSaveData[indexPath.row][2] == 0 ) || totalNum == tempIntList[indexPath.row][0] {
        ...
        //セーブデータの時間配置
        if tempSaveData[indexPath.row][1] > 9 {
