        ...
        for i in Num3 ..< taskStringListInst[Num1!].count {
            ...
            if taskIntListInst[Num1!][i][1] != 1 {
                ...
                if taskBoolListInst[Num1!][i][0] == true {
                    ...
                    //1/2を出す
                    halfNum = Double(taskIntListInst[Num1!][i][0] / 2)
                    //3/4を計算
                    threeQuatersNum = halfNum / 2
                    
                    //読み上げ時間：合計時間と足す
                    //経過時間計算
                    let progressTime: Double = Double(taskIntListInst[Num1!][i][0] - ((minNum * 60) + secNum))
                ...
                /*-------------▼完了時間予約▼-------------*/
                //(ラベルが白の時のみ)
                if plusLabel.isEnabled == false {
                    //タスク＋トータルタイムを足した値を時間予約
                    //もしi=Num3(初回)の時は【現在の残り時間】で予約
                    var reserveTime: Double = 0
                    if i == Num3 {
                        reserveTime = totalNum + Double((minNum * 60) + secNum)
                    }
                    //それ以外は合計時間＋そのタスク時間で予約
                    else {
                        reserveTime = totalNum + Double(taskIntListInst[Num1!][i][0])
                    }
            ...
            /*------------------------------▼タイマーストップ時▼------------------------------*/
                ...
                if i == Num3 {
                    totalNum += Double(( minNum * 60 ) + secNum)
                }
                //それ以外はそのタスク時間を代入
                else {
                    totalNum += Double(taskIntListInst[Num1!][i][0])
                }
            }
        }
    }
...
//        self.userActivity = userActivityType
    ...
    func reloadBackGround(){
        ...
        if foreChk == true {
            ...
            //先に完了ボタン押された判定入っていない時のみ進める、他はbreak（Num3>=.count）
...
//        taskLastRecord[Num1!][Num3][2] = recSec
        ...
        recMinTotal = recMinTotal + recMin
        recSecTotal = recSecTotal + recSec
