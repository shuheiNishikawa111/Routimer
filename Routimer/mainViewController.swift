...
class mainViewController: UIViewController, shortCutDelegate {
    ...
    override func viewDidLoad() {
        ...
        /*----------▼削除必須▼----------*/
        userDefaults.register(defaults: ["rNL": ["操作方法について"]])//routinNameList[]
        userDefaults.register(defaults: ["tSL": [[["○タップで編集○"]]]])//taskStringList
...
//        rankList.append([[100,0,0,0,0,0]])
        ...
        //初回起動時「」タスク追加する
        if initSwitch == true {
            taskStringListInst[0].append(["→右スワイプ【コピー・追加】→"])//右スワイプ説明
            taskStringListInst[0].append(["←左スワイプ【削除】←"])//左スワイプ説明
            taskStringListInst[0].append(["⚫︎長押しで順番入れ替え⚫︎"])//長押しスワイプ説明
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        var chkDelete: Bool = false
        
        for i in 0 ..< routinNameListInst.count {
            for j in 0 ..< taskIntListInst[i].count {
                if ( taskIntListInst[i].count == 1 && taskIntListInst[i][j][0] == 0 && taskIntListInst[i][j][1] == 0 && taskIntListInst[i][j][2] == 0 && taskIntListInst[i][j][3] == -1 && routinNameListInst[i] == "" ) {
                    //取り除く
                    routinNameListInst.remove(at: i)
                    taskStringListInst.remove(at: i)
                    taskIntListInst.remove(at: i)
                    taskBoolListInst.remove(at: i)
                    
                    taskTimeTotal.remove(at: i)
                    taskLastRecord.remove(at: i)
                    taskLastRecordTotal.remove(at: i)
                    taskSaveData.remove(at: i)
                    rankList.remove(at: i)
    ...
    func shortcutPlay(){
        ...
        if Num1! - 1 < routinNameListInst.count && Num1! > 0 {
            Num1 = Num1! - 1
            if taskStringListInst[Num1!].count > 0 {
    ...
    func createTemplate(){
        ...
        //ルーチン追加
        routinNameListInst.append("モーニングルーチン")
        taskStringListInst.append([["コップ一杯の水を飲む"]])
        ...
        taskStringListInst[1].append(["カーテンを開ける"])//
        taskStringListInst[1].append(["ベッドメーク"])//
        taskStringListInst[1].append(["軽いストレッチ"])//
        taskStringListInst[1].append(["洗顔"])//
        taskStringListInst[1].append(["朝食"])//
        taskStringListInst[1].append(["歯磨き"])//
        taskStringListInst[1].append(["着替え"])//
        
        //記録保持変数
        for _ in 0 ..< 7 {
            taskIntListInst[1].append([0,0,0,-1])
            taskBoolListInst[1].append([false])
            
            taskLastRecord[1].append([0])
            taskSaveData[1].append([0,0,0])
            rankList[1].append([100,0,0,0,0,0])
        }
        
        
        //保存とテーブルビュー読み込みは関数外で行う
...
extension mainViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //読み出し
        let routinNameListInst = self.userDefaults.array(forKey: "rNL") as! [String]
        //セルを定義
        let cell = tableView.dequeueReusableCell(withIdentifier: "mainCell", for: indexPath)
        
        if routinNameListInst[indexPath.row] == "操作方法について" {
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        let insertTaskSaveData = taskSaveData[sourceIndexPath.row]
        ...
        taskSaveData.remove(at: sourceIndexPath.row)
        ...
        taskSaveData.insert(insertTaskSaveData, at: destinationIndexPath.row)
